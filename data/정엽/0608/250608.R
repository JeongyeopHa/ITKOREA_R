...
#View(결과)
결과$x=c(70, 65, 80, 72, 68, 55, 50, 60, 52, 48) #컬럼추가=>필터x 단순추가시
#View(결과) #새로운 컬럼 x 추가
#View(data)

# 전처리도구
# install.packages('dplyr') 설치는 한번만
library(dplyr) #설치된 두구를 불러옵니다.(디플리알=>문법)

# mutate : 새로운 열 추가(필터 및 결과 추가할 때 디플리 사용)
...
# 1: 두 변수가 완벽하게 양의 선형 관계를 가짐. 즉, 한 변수가 증가할 때 다른 변수도 증가
...
성별_상관계수 = data %>% group_by(성별) %>% summarise(상관계수=cor(키,몸무게, use='complete.obs'))
print(성별_상관계수)
...
#col: 점 색깔 지정
#pch: 점 크기
plot(data$키, data$ 몸무게, main='산점도와 회귀선', xlab='x축', ylab='Y축', col = 'blue',pch=19)
