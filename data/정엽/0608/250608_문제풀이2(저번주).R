...
문제3 = data %>% filter(대여구분코드!='일일권(비회원)') %>% group_by(대여구분코드) %>% summarise(총이용건수 = sum(이용건수,na.rm=TRUE), 평균이용시간 = mean(`이용시간(분)`,na.rm=TRUE))
print(문제3)
...
문제6 = data %>% group_by(연령대코드) %>% summarise(이용건수=sum(이용건수,na.rm=TRUE), 평균운동량 = mean(운동량, na.rm=TRUE)) %>% arrange(desc(평균운동량)) %>% slice(1)
print(문제6)

#대여소명에 "역"이 포함된 대여소에서 발생한 총 운동량의 합 조회

text=c('apple', 'banana', 'grape', 'bread')
#'ap'가 들어간 단어 찾기
# grep(잡다) logicdal(TRUE, FALSE)
