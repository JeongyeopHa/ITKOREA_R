...
# dnorm : density의 d와 normal distribution(정규분포)의 rorm이 결합된 단어.
# dnorm 은 주어진 평균과 표준편차를 갖는 정규분포에서 특정 x에 대한 확률밀도를 계산.
# 데이터가 특정 구간에 있을 확률
y= dnorm(x,mu,sigma)
...
# plot(x,y, type = 'l', lwd=2,col='blue',main='학생점수정규분포',xlab='점수', ylab='확률')
...
print(students)
#2~3이면 평균에서 멀리있음.Z-SCORE (임계값) : 상황에 따라 조절하면 됨.

library(dplyr)
이상치제거_데이터=students %>% filter(z_score < 2) #2는 임계값, 상황에 따라 조절
print(이상치제거_데이터)
...
# 표준오차
# 표준 오차 : 내가 구한 평균값이 얼마나 믿을 만한지를 알려주는 숫자
# 표준 오차 작다 : 내가 구한 평균이 진짜 평균에 가까울 확률이 높다.
# 표준 오차 크다 : 내가 구한 평균이 진짜 평균과 많이 다를수 있다.
