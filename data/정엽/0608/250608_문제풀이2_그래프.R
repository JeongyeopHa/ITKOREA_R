...
#박스플롯
boxplot(운동량~성별, data=문제1, main='성별 운동량 박스플롯', xlab='성별',ylab='운동량', col = c('grey','pink','skyblue'))

# 문제 2: 연령대별 평균 이동거리(M)를 막대그래프로 표현.
문제2 = data %>% group_by(연령대코드) %>% summarise(평균이동거리 = mean(`이동거리(M)`, na.rm = TRUE))

#막대그래프
barplot(문제2$평균이동거리, names.arg = 문제2$연령대코드, main = '연령대 별 평균 이동거리',xlab='연령대',ylab='평균 이동거리')


#문제 4: 정기권을 구매한 이용자 중 연령대 별 평균 운동량 파이차트로 표현. 
# 단, 이용시간(분) 5분 이하는 평균에서 제외

문제4 = data %>% filter(대여구분코드 == '정기권' & `이용시간(분)`> 5) %>% group_by(연령대코드) %>% summarise(평균운동량 = mean(운동량, na.rm=TRUE))
...
#제문제5: 정기권 회원 중 연령대가 10대에서 50대 사이의 이용시간과 운동량을 비교하는 산점도그래프 표현하시오.(이용시간 x축, 운동량 y축) 회귀선까지 추가하시오.
