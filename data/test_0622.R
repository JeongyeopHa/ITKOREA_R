...
# head로 상위 데이터 확인
print(head(weather)) #상위 6개 행 데이터 확인
...
par(mfrow = c(2,3)) #2행 3열로 그래프 배치(단, 기본그래픽가능)
...
# 1. 시간별 기온 변화
# type='l' : line을 의미
#par(mfrow = c(1,3)) #1행 1열로 다시 나오게 복구
...
# 1.5 ? => 통계학자 `존 튜키` 제안한 것으로 
# 1.5수치는 너무 좁지도, 너무 넓지도 않은 적잘한 범위를 설정하기 위해
# 표준으로 사용하고 있습니다.
lower_bound = Q1 - 1.5 * IQR
upper_bound = Q3 + 1.5 * IQR


# mutate: 컬럼 수정 or 추가할 때
# 이상치 데이터는 NA로 임시처리
weather = weather %>% 
...
# 5월 1일부터 ~ 5월 31일 대전 지점 데이터 필터링(디플리알)
# 대전 5월날씨 가져옴
...
# 5. 실용적 분석
# 시간마다 기록된 숫자 => `시계열 데이터`
# 시계열 분석은 이 숫자들이 어떻게 바뀌는지 어떤 규칙이 있는지 알아볼 수 있습니다.
...
library(forecast)
auto_model = auto.arima(temp_ts)#시계열 데이터를 바탕으로 미래를 예측하는 통계 모델
#향후 30시간 기온 예측
